          ...
          get: function get(obj2, apiCall) {
            return function call() {
              ...
              return schemaPromise.then(function (schema) {
                ...
                var bodyOrQueryString = args.shift() || {};
                ...
                var deserialize = function deserialize(response) {
                  return deserializeResponse ? (0, _deserializeJsonApi.default)(singularized, link, response) : response;
                };

                var serializeRequest = Object.prototype.hasOwnProperty.call(options, 'serializeRequest') ? options.serializeRequest : true;

                if (link.schema && (link.method === 'PUT' || link.method === 'POST') && serializeRequest) {
                  bodyOrQueryString = (0, _serializeJsonApi.default)(singularized, bodyOrQueryString, link, link.method === 'PUT' && lastUrlId);
                }

                if (link.method === 'POST') {
                  return rawClient.post("".concat(url), bodyOrQueryString).then(function (response) {
                    return deserialize(response);
                  });
                }

                if (link.method === 'PUT') {
                  return rawClient.put("".concat(url), bodyOrQueryString).then(function (response) {
                    return deserialize(response);
                  });
                }
                ...
                var request = allPages ? (0, _fetchAllPages.default)(rawClient, url, bodyOrQueryString) : rawClient.get(url, bodyOrQueryString);
