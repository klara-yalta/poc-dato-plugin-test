...
function deserialize(type, relationshipsMeta, schema, _ref3) {
  ...
  var attrs = type === 'item' ? Object.keys(attributes).map(function (key) {
    return {
      key: key,
      details: null
    };
...
function deserializeJsonApi(type, link, json) {
  if (!json) {
    return json;
  }
  ...
  if (Array.isArray(data)) {
    return data.map(function (item) {
      return deserialize(type, relationshipsMeta, link.targetSchema, item);
    });
  }
